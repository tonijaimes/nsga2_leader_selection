...
void NSGA::optimize()
{
   ...
   cerr << "\nSe consiguió pasar report 2.\n";


   for (int i=2; i<=ngen; i++)
   {
       selection (parent_pop, child_pop);
       cerr << "\nSe consiguió pasar selection.\n";

       mutation_pop (child_pop);
       cerr << "\nSe consiguió pasar mutation.\n";

       decode_pop(child_pop);
       cerr << "\nSe consiguió pasar decode.\n";

       evaluate_pop(child_pop);
       cerr << "\nSe consiguió pasar evaluate.\n";

       merge (parent_pop, child_pop, mixed_pop);
       cerr << "\nSe consiguió pasar merge.\n";
...
void NSGA::bincross(Individual *parent1, Individual *parent2, Individual *child1, Individual *child2)
{
   ...
   for (i=0; i<nbin; i++)
   {
       ...
       if (rand <= pcross_bin)
       {
           ...
           cerr << "site1= " << site1 << "site2= " << site2;
           for (j=0; j<site1; j++)
           {
               cerr << "\n\nsize gene padre 1= " << parent1->gene[0].size();
               cerr << "\nsize gene padre 2= " << parent2->gene[0].size();
               cerr << "\nsize gene hijo 1= " << child1->gene[0].size();
               cerr << "\nsize gene hijo 2= " << child2->gene[0].size();
